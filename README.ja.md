...
Claude Code、Codex CLI、OpenCodeなどのAIコーディングツールは、agents、skills、hooks、pluginsといったオーケストレーションレイヤーをサポートしています。代表的なオーケストレーションパッケージには**omc**、**ecc**、**bkit**、**superpowers**などがあります。

問題点：**一度に使えるオーケストレーションレイヤーは1つだけ**であり、切り替えには`~/.claude/`や`~/.config/opencode/`のファイルを手動で移動する必要があります。

**agent orchester**はオーケストレーションプロファイルを分離管理し、美しいターミナルUIでワンキー切替を実現します。
...
- **プラグインサポート** — プラグインタイプのインストール（`/plugin install`など）を認識し、TUIで手動コマンドを表示。ハイブリッドプロファイルはsymlinkとプラグインを組み合わせ
- **自動ロールバック** — 切替中にエラーが発生した場合、自動的に前の設定を復元
- **ビルトインレジストリ** — omc、ecc、bkit、superpowersなどの人気パッケージをTUIから直接インストール
...
初回起動時、agent orchesterは以下を実行します：
...
```
╭──────────────────────────────────────────────────╮
│ agent orchester v0.2  プロファイル管理             │
├──────────────────────────────────────────────────┤
│ ランタイム  claude ✓  codex ✓  opencode ✗        │
├──────────────────────────────────────────────────┤
│ ● omc — 28エージェント、37スキル [orchestration]  │
│ ○ ecc — 包括的スターターキット                    │
│ ○ superpowers 🔀 — TDDワークフロー、マルチツール  │
│ ○ none（バニラ）                                  │
├──────────────────────────────────────────────────┤
...
agent orchesterは**2フェーズモデル**を使用します：

### フェーズ1：インストール
オーケストレーションパッケージを`~/.orchester/profiles/`にダウンロード。ファイルをローカルに保存するだけで、**アクティブ化はしません**。

### フェーズ2：アクティブ化
プロファイルのファイルからターゲットツールの設定ディレクトリ（例：`~/.claude/agents/`）へsymlinkを作成。一度にアクティブにできるプロファイルは1つだけです。プラグインタイプのリンクの場合、AIコーディングツールで実行する手動コマンドを表示します。
...
| タイプ | アイコン | 説明 |
|--------|---------|------|
| `symlink` | （デフォルト） | 標準のsymlinkベースインストール |
| `plugin` | 🔌 | 手動実行用プラグインコマンドを表示 |
| `hybrid` | 🔀 | symlinkとプラグインコマンドの組み合わせ |
...
| superpowers | 53.5K | Agenticスキルフレームワーク、TDDワークフロー | Claude Code |
...
- **v0.3** — MCP名前空間分離、プロジェクト別プロファイル、プロファイル合成（ミックス＆マッチ）
