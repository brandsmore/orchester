...
| 名前 | 説明 | ツール |
|------|------|--------|
| omc | 5モード、32エージェント、31+スキル、HUD | Claude Code |
| ecc | 包括的スターターキット | Claude Code |
| bkit | PDCAベースAIネイティブワークフロー | Claude Code |
| wshobson-agents | 73プラグイン、112エージェント、マーケットプレイス | Claude Code |
| oh-my-opencode | Sisyphusオーケストレーター、25+フック | OpenCode |
| claude-orchestra | 40+エージェント 組織図構造 | Claude Code |
...
- **v0.2** — MCP名前空間分離、プロジェクト別プロファイル
- **v0.3** — プラグインシステム、プロファイル合成（ミックス＆マッチ）
